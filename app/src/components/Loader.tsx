...
// Определим типы для TSX
const Loader: React.FC = () => {
  useEffect(() => {
    const select = (s: string) => document.querySelector(s) as SVGSVGElement;
    const toArray = (s: string) => Array.from(document.querySelectorAll(s)) as SVGElement[];
    ...
    function animate(el: SVGElement, count: number) {
    ...
    allEll.forEach((el, i) => {
      gsap.delayedCall(i / (allEll.length - 1), () => animate(el, i + 1));
    });
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: '#000' }}>
      <svg id="mainSVG" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600">
        ...
        {Array.from({ length: 30 }).map((_, index) => (
          <ellipse key={index} className="ell" cx="400" cy="300" rx="80" ry="80" />
        ))}
        <path
          id="ai"
          opacity={0.85}
