...
import { createWorkspace, getWorkspaces, Workspace,deleteWorkspace,updateUserProfile } from "../api"; // Импортируем API-запросы
...
interface UserProfileProps {
  user: {
    ...
    age: number;
    username: string;
    avatar: string | File; // Аватар может быть строкой (URL) или файлом;
...
const UserProfile: React.FC<UserProfileProps> = ({ user }) => {
  ...
  const [formData, setFormData] = useState(() => {
    if (user) {
      const { id, ...rest } = user; // Убираем id из user
      return rest;
    }
    return null;
  });
  ...
  useEffect(() => {
    if (user) {
      const { id, ...rest } = user; // Исключаем id
      setFormData(rest);
      if (isCurrentUser) {
        fetchUserWorkspaces();
      }
  ...
  const handleAddWorkspace = async () => {
    if (newWorkspaceName.trim()) {
      try {
        ...
        setServerWorkspaces([...serverWorkspaces, newWorkspace]);
  ...
  const handleDeleteWorkspace = async (workspaceName: string) => {
    try {
      ...
      setServerWorkspaces((prevWorkspaces) =>
